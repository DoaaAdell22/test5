...
const Friends = () => {
    ...
    const removeCard = (e , clickedIdx) =>{
        const deleter = slides.filter((el , idx) => idx !== clickedIdx);
        setSlides(deleter);
    }
    ...
    return (
        <div>
            ...
            <div className="Box-friend">
                { slides.map((el , idx) => <Slidefriends handler={removeCard} img = {el.image} name = {el.name} job = {el.job} info1 = {el.info1} info2 = {el.info2} info3 = {el.info3} Date = {el.Date} idx = {idx} /> ) }
            </div>
        </div>
    )
}
